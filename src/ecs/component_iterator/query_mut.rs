...
#[macro_export]
macro_rules! generate_query_mut {
    ($name:ident, $state_name:ident, $($types:tt),+) => {
        ...
        paste::paste! {
            ...
            pub struct $name<'a, $($types),+> {
                ...
                $(
                    pub [<$types:snake>]: std::iter::Peekable<std::slice::IterMut<'a, FoundryIndexedElemInner<$types>>>,
                )+
            }

            impl<'a, $($types),+> Iterator for $name<'a, $($types),+> {
                #[allow(unused_parens)]
                type Item = (usize, $(&'a mut $types),+);
                        ...
                        match self.query_state {
                            ...
                            $state_name::End => {
                                let result = (
                                    self.current_entity,
                                    $(
                                        self.[<$types:snake>].next()?.elem_mut()
                                    ),+
                                );
                                self.current_entity += 1;
                                self.query_state = $state_name::reset_state();
...
macro_rules! impl_query_mut {
    ($name:ident, $state_name:ident, $($types:tt),+) => {
        impl crate::ComponentTable {
            paste::paste! {
                pub fn [<$name:snake>]<'a, $($types: 'static),+>(&'a self) -> impl Iterator<Item = (usize, $(&'a mut $types),+)> {
                    unsafe {
                        $name {
                            current_entity: 0,
                            mask: u32::MAX,
                            ...
                            $(
                                [<$types:snake>]: match self.get_component_array_mut::<$types>() {
                                    Some(arr) => arr.iter_mut().peekable(),
